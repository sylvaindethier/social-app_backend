const FILE_PATH = "./app/domains/user/model";

import { Schema, model } from "mongoose";
import type { Model, Document } from "mongoose";

// Role
import { Role } from "#app/domains/role/model.ts";
import type { RoleInterface, RoleName } from "#app/domains/role/model.ts";
...
async function from_password({ password, ...rest }: UserWith_password) {
  const passwordHash = await password__hash(password);
